...
syn keyword cobolKeyword   contained accept add all assign and call cancel close compute commit continue copy delete delimited display divide else end-call end-if end-evaluate end-perform
...
syn keyword cobolKeyword   contained replacing rounded search select set size sort subtract start stop string to trailing unstring unlock until using varying when write .
...
syn keyword cobolKeyword   contained delaratives delimiter depending descending destination
...
syn keyword cobolKeyword   contained global greater group heading high-value high-values i-o
syn keyword cobolKeyword   contained identification index indexed indicate initial
syn keyword cobolKeyword   contained initiate input input-output installation i-o-control just
...
syn keyword cobolKeyword   contained transaction true type unit up upon usage use value values
